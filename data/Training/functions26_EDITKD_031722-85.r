...
{ ## LISTS
   ...
   lfMetaColums  <- c("sample","number","group")
...
{ ## HELPER FUNCTIONS
   ...
   ##----------------------------------
   ##    BIOMART ARCHIVE ANNOTATION
   ##----------------------------------
   ## db.version=103, genome.assembly="GRCh38.p13", id.type="ENSEMBL", gene.ids=rownames(counts)
   ## ensembl archive page: http://uswest.ensembl.org/info/website/archives/index.html
   ## click on link in right panel. select organism of interest from "all genomes dropdown".
   ## click More information and statistics link. get assembly and database version info. from right table
   ## https://davetang.org/muse/2013/05/23/using-the-bioconductor-annotation-packages/
   ...
   ## https://davetang.org/muse/2012/04/27/learning-to-use-biomart/
   ...
   ## genecode versions: https://www.gencodegenes.org/human/releases.html
   ## https://may2012.archive.ensembl.org/Homo_sapiens/Info/Index?db=core
   ## db.version="current" or 97, genome.assembly="GRCh38" or "GRCm38", 
   ## from.type="ENSEMBL" or "ENTREZID" or "UNIPROT" or "uniprotswissprot" or "ensembl_gene_id"
   ## gene.ids = character vector of ids matching from.type
   ## to.type=NULL (retrieves default set of attributes, some ensembl,entrez,uniprot,mirbase) 
   ## or character vector of 
   ## attributes to retrieve e.g. "description", "chromosome_name", "uniprot_gn_symbol", "mirbase_id"
   get_biomart_annot <- function(db.version="current", genome.assembly="GRCh38", from.type="ENSEMBL", 
                                 gene.ids, to.type=NULL){
      
      param<-stats<-list()
      
      getOption('timeout')  ## default timeout is 60s
      options(timeout=1200) ## change timeout to 600s i.e. 10min to allow download large file
      getOption('timeout')
      
      ## get avaiable ensembl db verisons
      archives <- biomaRt::listEnsemblArchives();  print(head(archives))
      
      ## current db version
      cur<-archives$current_release=="*";cur
      cur.db.version <- archives$version[cur==TRUE];cur.db.version
      
      ## get ensembl database version from archive
      if(db.version=="current"){db.version<-cur.db.version};db.version
      if(is.numeric(db.version)){db.version<-as.character(db.version)}
      
      ## if db.version is invalid return data.frame of available info. and save to 
      ## info. to file in project directory.
      if(db.version%in%archives$version==FALSE){
         if(!dir.exists("logs")){dir.create("logs",recursive=TRUE) }
         write.csv(archives, file="./logs/ERROR_biomart_listEnsemblArchives.csv")
         message("\nERROR! invalid db.version. see version column of listEnsemblArchives in returned output",
                 "or output file ./logs/ERROR_biomart_listEnsemblArchives.csv");cat("\n\n")
         out=list(listEnsemblArchives=archives)
         return(out)
      }
      
      if(db.version==cur.db.version){ host="www.ensembl.org" }
      if(db.version !=cur.db.version){
         host=archives[grep(db.version,archives$version),"url"];host
         # host=gsub("https://","",host);host
      }
      db.version;host
      
      
      ## get Genes database for use with biomart
      print(biomaRt::listMarts(host=host));cat("\n\n\n")
      genes.biomart=biomaRt::listMarts(host=host)[grep("Genes",biomaRt::listMarts(host=host)$version),"biomart"]
      genes.biomart
      
      ## get all available organism datasets from the ensembl genes database
      organism.datasets=biomaRt::listDatasets(biomaRt::useMart(biomart=genes.biomart,host=host))
      head(organism.datasets)
      
      ## get row number corresponding to input genome.assembly value
      rw<-grep(genome.assembly,organism.datasets$version);rw
      ## if input genome.assembly info is not listed then save all organism dataset info to file.
      ## see version column 
      if(length(rw)!=1){
         if(!dir.exists("logs")){dir.create("logs",recursive=TRUE) }
         write.csv(organism.datasets, "./logs/ERROR_biomart_listDatasets.csv")
         message("\nERROR! invalid geneome.assembly. see version column of listDatasets in returned output",
                 " or output file ./logs/ERROR_biomart_listDatasets.csv");cat("\n\n")
         out=list(listEnsemblArchives=archives, listMarts=biomaRt::listMarts(host=host), listDatasets=organism.datasets,
                  db.version=db.version, genes.biomart=genes.biomart,host=host)
         return(out)
      }
      
      my.assembly<-organism.datasets$version[rw];my.assembly
      my.dataset <- organism.datasets[grep(my.assembly, organism.datasets$version),"dataset"];my.dataset
      print(my.assembly);print(my.dataset)
      
      ## connect to genes mart for organism dataset from ensembl archive database
      my.mart = biomaRt::useMart(biomart=genes.biomart,dataset=my.dataset,host=host); print(my.mart)
      
      ## set the id type for your list of gene ids
      filters<-biomaRt::listFilters(my.mart)
      from.type=match.arg(arg=from.type,choices=c("ENSEMBL","ENTREZID","UNIPROT",filters$name), 
                          several.ok=FALSE);from.type
      
      if(from.type=="ENSEMBL"){
         tmp<-filters[filters$name %in% "ensembl_gene_id",];tmp
         if(nrow(tmp)==1){ from.type <- tmp$name };from.type 
      }
      
      if(from.type=="ENTREZID"){
         tmp<-filters[filters$name %in% "entrezgene_id",];tmp
         if(nrow(tmp)==1){ from.type <- tmp$name };from.type
      }
      
      if(from.type=="UNIPROT"){
         tmp <- filters[filters$name %in% "uniprotswissprot",];tmp
         if(nrow(tmp)==1){ from.type <- tmp$name };from.type
      }
      
      if(from.type%in%filters$name){
         tmp<-filters[filters$name %in% from.type, ];tmp
         if(nrow(tmp)==1){ from.type <- tmp$name };from.type
      }
      print(paste("from.type:",from.type))
      
      
      ## get all available attributes
      attributes <-biomaRt::listAttributes(mart=my.mart);head(attributes)
      # grep("^ensembl_gene",attributes$name,ignore.case=TRUE, value=TRUE)
      # grep("^uniprot",attributes$name,ignore.case=TRUE, value=TRUE)
      # grep("^entrez",attributes$name,ignore.case=TRUE, value=TRUE)
      
      
      ## get default attribute columns
      if(is.null(to.type)){
         my.attributes <- c(from.type,"hgnc_symbol","external_gene_name","description","chromosome_name",
                            "gene_biotype",
                            "entrezgene_id","entrezgene_accession","entrezgene_description",
                            "uniprotswissprot","uniprot_gn_symbol","mirbase_accession","mirbase_id")
      }
      if(!is.null(to.type)){
         if(all(to.type%in%attributes$name)==TRUE){ my.attributes<-c(from.type,to.type) }
         if(all(to.type%in%attributes$name)==FALSE){
            write.csv(attributes, "ERROR_biomart_listAttributes.csv")
            message("ERROR! invalid to.type attributes: ",paste(to.type[to.type%in%attributes$name==FALSE],collapse=","))
            message("see name column of listAttributes in the returned output for valid attribute ids",
                    "\nfor the output ERROR_biomart_listAttributes.csv");cat("\n\n")
            out=list(listEnsemblArchives=archives, listMarts=listMarts(host=host), 
                     listDatasets=organism.datasets, listFilters=filters,
                     listAttributes=attributes, db.version=db.version, host=host,
                     genome.assembly=my.assembly, biomart.database=genes.biomart,
                     dataset=my.dataset)
            return(out)
         }
      }
      
      attr.info<-data.frame(subset(attributes[attributes$name %in% my.attributes,], 
                                   attributes[attributes$name %in% my.attributes,]$page=="feature_page"))
      m<-match(my.attributes,attr.info$name);m
      attr.info<-attr.info[m,]
      cat("\n\n");print(attr.info);cat("\n\n")
      
      ##---------------------------------------------------------------- 
      ## BIOMART QUERY : RETRIEVE ATTRIBUTE ANNOTATION RESULTS
      ##----------------------------------------------------------------
      ## create df of the input gene.ids. this df is merged with biomart query results (left join)
      ## this ensures that a row for each gene.id is present even if no annotation info was returned
      df<-df2<-df3<-data.frame(gene.ids=gene.ids)
      
      ## create biomart queries and retrieve info for each attribute in the list separately
      for(i in 2:length(my.attributes)){
         
         attr=c(my.attributes[1], my.attributes[i]);attr
         my.query<-biomaRt::getBM(filters=from.type, attributes=attr, values=gene.ids, mart=my.mart); 
         head(my.query);dim(my.query)
         
         ## replace blanks with NA and remove arbitrary text from description
         ## i.e. everything after [      
         my.query[,2] <- gsub(" \\[.*","",my.query[,2]);head(my.query)
         my.query[my.query==""]<-NA;head(my.query)
         
         ## query results for each attribute 1:many mappings are merged together
         ## into a single dataframe
         ## biomart query results of first attribute is merged (left join) with
         ## the initial df containing all input gene.ids 
         ## this ensures that a row for each gene.id is present even if no annotation info was returned
         ## query results for each subsequent attribute are left jointed to the data frame
         ## data with include 1:many mappings many duplicated rows
         df<-merge(x=df, by.x=colnames(df)[1], y=my.query, by.y=colnames(my.query)[1],all.x=TRUE)
         df<-data.frame(df); head(df);dim(df)
         
         
         o=order(rowSums(is.na(my.query)),decreasing=FALSE)
         my.query3<-my.query[o,]
         df3<-merge(x=df3, by.x=colnames(df3)[1], y=my.query3, by.y=colnames(my.query3)[1],all.x=TRUE)
         df3<-data.frame(df3); head(df3);dim(df3)
         
         ## GET FIRST ENTRY IE ATTRIBUTE PRIMARY ID  
         ## biomart query will return 1:M mapping between the gene.ids and attributes requested
         ## this is because one ensembl gene id may map to multiple entrez genes or multiple proteins,
         ## or several gene symbols. some ensembl ids will not return any annot info.it may be a
         ## predicted gene that does not overlap with any entrez/refseq gene transcript models
         ## Determine the indices for the non-duplicated genes
         
         ## To prevent 1:mapping relations only the first ID is returned.
         ## the first entry is the primary id. to do this ...
         ## rows are sorted (decreasing=F L-> H) by the number of NAs 
         ## keeping the original order of the duplicated gene.ids/attrib.ids
         ## (basically shift all the rows with a bunch of NAs to the bottom and 
         ## moves the primary attribute id of interest to the top. so it is the
         ## first row entry for the set of rows with duplicate gene.ids.
         ## duplicate rows are then removed,(only the first row is kept)
         o=order(rowSums(is.na(my.query)),decreasing=FALSE)
         query.sort <-my.query[o,];head(query.sort)
         
         # Return only the non-duplicated genes 1:1 mapping gene.id:primary attr.id
         query.nodups <- query.sort[!duplicated(query.sort[,1]),];dim(query.nodups)
         
         
         ## merge 1:1 mapping between gene.id and first entry primary att. id (no dup rows)
         ## with df2 (1 row per gene.id no duplicate rows)
         df2<-merge(x=df2, by.x=colnames(df2)[1], y=query.nodups, by.y=colnames(query.nodups)[1],all.x=TRUE)
         df2<-data.frame(df2); head(df2);dim(df2)
         
         
      }  ## FOR LOOP BIOMART QUERY RESULTS
      
      
      if(all(!duplicated(df[,1]))){ rownames(df)<-df[,1] }
      if(all(!duplicated(df2[,1]))){ rownames(df2)<-df2[,1] }
      
      param[["db.version"]]<-db.version
      param[["host"]]<-host
      param[["genome.assembly"]]<-my.assembly
      param[["biomart.db"]]<-genes.biomart
      param[["dataset"]]<-my.dataset
      param[["from.type"]]<-from.type
      
      if(is.null(to.type)){
         
         param[["to.type"]]<-paste(my.attributes[1:6],collapse="; ")
         param[["to.type2"]]<-paste(my.attributes[7:9],collapse="; ")
         param[["to.type3"]]<-paste(my.attributes[10:11],collapse="; ")
         param[["to.type4"]]<-paste(my.attributes[12:13],collapse="; ")
         
         stats[["gene.ids"]] <- length(gene.ids)
         stats[["annot.first"]] <- nrow(df2)
         stats[["annot.all"]] <- nrow(df)
         
      } 
      
      if(!is.null(to.type)){
         
         param[["to.type"]]<-paste(my.attributes,collapse="; ")
         stats[["gene.ids"]] <- length(gene.ids)
         stats[["annot.first"]] <-nrow(df2)
         stats[["annot.all"]]<-nrow(df)
      ...
      logs <- make_log(param=param, stats=stats, title="get_biomart_annot()", save=TRUE)
      
      biomart.objects <-list(listEnsemblArchives=archives, listMarts=biomaRt::listMarts(host=host), 
                             listDatasets=organism.datasets, listFilters=filters,
                             listAttributes=attributes, db.version=db.version, host=host,
                             genome.assembly=my.assembly, biomart.database=genes.biomart,
                             dataset=my.dataset,gene.ids=gene.ids,from.type=from.type,to.type=my.attributes)
      
      
      data2 <- list(annot.first=df2, annot.all=df, 
                    attr.info=attr.info, param=logs$param, stats=logs$stats,
                    biomart.objects =biomart.objects )#, stats=logs$stats)
      
      print("BioMart annotation retrieval complete. Success!");cat("\n\n")
      return(data2)
   ...
   ##----------------------------------
   ##  ORGANISM PACKAGE ANNOTATION
   ##----------------------------------
   ## OrgDb must be character "Org.Hs.eg.db"; 
   ## geneIDs=vector of ids, fromType=id type for ids listed in geneIDs c() 
   ## toType=c() vector of desired id types to retrieve. must match package keytpes
   ## Note only the first instance is returned.
   get_pkg_annot <- function(geneIDs, OrgDb="Org.Hs.eg.db", fromType="ENSEMBL", 
                             toType=c("ENTREZID","SYMBOL","GENENAME")){
      
      param <- stats <- list()
      
      if(class(OrgDb) != "character"){
         stop("\nERROR! OrgDb should be a character string identifying the DB 
                     package e.g. OrgDb='org.Hs.eg.db' . Exiting...")
      }
      
      if(class(OrgDb)=="character"){
         
         ## if package not installed. install package
         if(OrgDb %in% c(rownames(installed.packages()))==FALSE){
            print(paste("installing",OrgDb,"..."))
            BiocManager::install(OrgDb, force=TRUE)
            print(paste(OrgDb,"installed... Success!"))
         }
         
         ## if package is in list of installed package load package
         if(OrgDb %in% rownames(installed.packages())){
            
            print(paste(OrgDb,"package is installed. Loading package..."))
            suppressPackageStartupMessages(library(OrgDb,character.only=TRUE, quietly=TRUE))
            
            cat("---------------------\n")
            cat("\nPackage Keytypes:\n")
            cat("---------------------\n")
            print(keytypes(get(OrgDb)))
            cat("\n\n---------------------\n")
            cat("Objects In Package:\n")
            cat("---------------------\n")
            print(ls(paste0("package:",OrgDb)))
            cat("\n\n---------------------\n")
            cat("DB Sources In Package:\n")
            cat("---------------------\n")
            tmpfile=tempfile(pattern="file",tmpdir=tempdir(),fileext=".txt");tmpfile
            sink(file=tmpfile);print(get(OrgDb));sink()
            pkgInfo=read.delim(file=tmpfile,header=FALSE,skip=1)
            pkgInfo= strsplit(unlist(sapply(1:nrow(pkgInfo),FUN=function(x){ gsub("\\| ","",pkgInfo[x,])})),split=": ")
            pkgInfo2<-matrix(ncol=2,nrow=length(pkgInfo))
            pkgInfo2[,1]<-sapply(1:length(pkgInfo),FUN=function(x){pkgInfo[[x]][1]})
            pkgInfo2[,2]<-sapply(1:length(pkgInfo),FUN=function(x){pkgInfo[[x]][2]})
            pkgInfo2<-data.frame(pkgInfo2);colnames(pkgInfo2)<-NULL
            pkgInfo2<-rbind(c("PACKAGE",OrgDb),pkgInfo2);head(pkgInfo2)
            cat("\n\n");print(pkgInfo2);cat("\n\n")
            
            if(!dir.exists("logs")){dir.create("logs",recursive=TRUE)}
            write.table(x=pkgInfo2, file="./logs/organism_package.txt", quote=TRUE, sep="\t", 
                        col.names=FALSE, row.names=FALSE)
            
            ## make sure entered toType values are valid key types for the package.
            fromType <- match.arg(fromType, keytypes(get(OrgDb)),several.ok=FALSE);fromType
            toType   <- match.arg(toType, keytypes(get(OrgDb)),several.ok=TRUE); toType
         }
      } 
      
      ## use keyType to get annot annotation for only firt entry i.e 1:MANY not returned
      ## only 1:1 using first instance.
      annot <- matrix(nrow=length(geneIDs), ncol=(1+length(toType)));head(annot)
      colnames(annot) <- c(fromType,toType)
      annot[,1] <- geneIDs
      annot<-data.frame(annot);head(annot)
      
      ## add annotation to specific columns and set rownames as fromType i.e. geneIDs
      ## first == This value means that when there are multiple matches only the 
      ## 1st thing that comes back will be returned
      for(x in toType){
         annot[,x] <- AnnotationDbi::mapIds(x=get(OrgDb), keys=geneIDs, keytype=fromType, 
                                            column=x, multiVals="first")
      }
      rownames(annot)<- annot[,fromType]
      
      param[["OrgDb"]] <- OrgDb
      param[["fromType"]] <- fromType
      param[["toType"]] <- paste(toType, collapse="; ")
      stats[["total_no_annot_cols"]] <-ncol(annot)
      stats[["total_no_annot_rows"]] <- nrow(annot)
      
      logs <- make_log(param=param, stats=stats, title="get_pkg_annotation()", save=TRUE)
      
      # print(head(annot));print(dim(annot))
      data2<-list(annot=annot, pkgInfo=pkgInfo2, param=logs$param,stats=logs$stats)
      return(data2)
...
{ ## MAIN FUNCTIONS
   ...
   extract_data <- function(file=NULL, sampleIDs=NULL, min.prob=0.75,
      ...
      if(pipe=="DIA" & enrich=="protein"){
         ...
         utils::write.csv(bqData, file.path("logs",filename), row.names=FALSE)
   ...
   ##------------------------------------------
   ##  [10] GET DDA SAMPLE NUMBER (REQUIRED)
   ##------------------------------------------
   get_dda_sample_number <- function(sampleIDs){
      
      ## proteinGroups.txt should contain 
      ## iBAQ. as part of the sampleIDs
      pattern <- "iBAQ.[[:digit:]]"
      pat <- grep(pattern, sampleIDs, value=TRUE);pat
      
      if(all(length(pat)==length(sampleIDs))==TRUE){

         number <- as.numeric(gsub("iBAQ\\.","", sampleIDs));number ## removes iBAQ.
         print("sample numbers extracted from LF sampleIDs. Success!!")
         return(number)
      }
      
      if(all(length(pattern)!=length(sampleIDs))==FALSE){
         stop(message(paste("Error! Sample numbers could not be extracted", 
                            "from the input sampleIDs. LF sampleIDs should",
                            "contain the following patterns:", pattern,
                            "(pool samples e.g. iBAQ.01, iBAQ.33 ")))
      } 
      
   } ## GET SAMPLE NUMBER LF
   
   
   ##---------------------------------
   ##  [11] MAKE TARGETS (OPTIONAL)
   ##---------------------------------
   ## if meta data file not is not defined then the targets info extracted from the input sampleIDs
   ## is used to create basic targets df that can be combined with metadata info sample names, group etc.
   make_targets <- function(file=NULL, sampleIDs, pipe=c("DIA","LF","TMT","phosphoTMT"),
                            enrich=c("protein","phospho")){
      ...
      if(!is.null(file)){
         ...
         if(pipe=="LF"){ ## LF
            ...
            ## extract sample number from sampleIDs and use as data key
            number <- get_dda_sample_number(sampleIDs=sampleIDs);number
         ...
         if(pipe=="TMT" | pipe=="phosphoTMT"){ ## TMT/phosphoTMT
            ...
            if(all(rownames(targets) %in% rownames(metadata))){
               ...
               if(!dir.exists("logs")){dir.create("logs")}
               if(pipe=="phosphoTMT" & enrich=="protein"){ filename <- "./logs/targets.pro.csv" }
               if(pipe=="phosphoTMT" & enrich=="phospho"){ filename <- "./logs/targets.phos.csv" }
               if(pipe=="TMT" & enrich=="protein"){ filename <- "./logs/targets.csv" }
               utils::write.csv(targets, file=filename, row.names=FALSE)
      ...
      if(is.null(file)){
         ...
         if(pipe=="LF"){ ## LF NULL
            ...
            rownames(targets)<-targets$sampleIDs;head(targets)
            print(paste(enrich, "targets template for", pipe, "experiment created. Success!!",
                        "Import metadata (sample,number,group,etc) and combine."))
...
{ ## PROTEINORM QC PLOT FUNCTIONS
   ...
   ##  BOTTOM MARGIN LINES
   ##------------------------
   bottom_margin<-function(sampleLabels, cex.names=1){
      
      ## use nchar of sampleLabels to determine how many lines to increase margin in mar?
      in2px <- par("cin")/par("cra");in2px   ## no. inches per pixel
      px2in <- par("cra") / par("cin");px2in ## no. pixels per inch
      w.line.px <- par("cra")[1]; w.line.px  ## line width in pixels
      h.line.in <- par("csi");h.line.in      ## line height in inches
      h.line.px <- par("cra")[2]; h.line.px  ## line height in pixels
      
      ## determine string width/height in pixels of the longest sampleLabel
      ## length of sampleLabels (pixels) converted to number of lines to add to bottom margin
      long.str <- sampleLabels[nchar(sampleLabels)%in%max(nchar(sampleLabels))][1];long.str
      w.str.px <- strwidth(long.str,units="inches",cex=cex.names)/in2px[1];w.str.px ## strwidth() sums the lens of the ind. char in string
      h.str.px <- strheight(long.str,units="inches",cex=cex.names)/in2px[1];h.str.px ## width ie length of sampleLabels
      
      x1 <- (ceiling(w.str.px/h.line.px)+0.5)*1.25;print(x1) ## par(mar=# lines bottom)
      return(x1)
   ...
   ##  [12] plotTotInten 
   ##------------------------------
   plotTotInten <- function(normList, groups, batch=NULL, sampleLabels=NULL, cex.names=NULL, dir=".", save=FALSE){
      
      groups<-make_factor(groups);groups
      if(is.null(batch)){ batch <- c(rep("1",ncol(normList[[1]]))) };batch
      batch<-make_factor(as.character(batch),prefix=NULL)
      if(is.null(sampleLabels)){ sampleLabels <- colnames(normList[[1]]) };sampleLabels
      cex.names <- ifelse(is.null(cex.names),1,cex.names);cex.names
      ...
      ## BARPLOT TOTAL INTENSITY
      if(save==TRUE){ if(!dir.exists(dir)){dir.create(dir,recursive=TRUE)} }
      if(save==TRUE){png(filename=file.path(dir,"TotIntenPlot.png"), units="px",
                         width=width, height=height, pointsize=15)}

      ## no. lines added to bottom plot margin to fit sample names
      x1 <- bottom_margin(sampleLabels,cex.names);x1
      par(mar=c(x1,6,3,2),oma=c(0,0,2,1),mfrow=mfrow, xaxs="i", yaxs="i")
      print(paste("bottom margin lines:",x1))

      barList<-NULL
      for(i in names(normList)){
         barList[[i]] <- colSums(normList[[i]],na.rm=TRUE)
         barplot(barList[[i]],
                 main="", las=2, yaxt="n",
                 cex.names=cex.names,
                 col=colorGroup2(groups)[groups], names.arg=sampleLabels)
         title(main=i, font.main=1, cex.main=1.7, line=2)
         axis(side=2, cex.axis=1, las=2)
         abline(h=max(barList[[i]]), lty=2)
   ...
   plotNaHM <- function(normList, groups, batch=NULL, sampleLabels=NULL, dir=".", save=FALSE){
      ...
      width2=width/150;width=width/72 }
   ...
   ##  [19] BOX PLOT 
   ##------------------------------
   plotBoxplot <- function(data, groups=NULL, sampleLabels=NULL, cex.names=1, title=NULL, legend=TRUE, dir=".", save=FALSE){
      ...
      ## plot margins
      x1<-bottom_margin(sampleLabels,cex.names=cex.names)
      x2<-bottom_margin(sampleLabels=as.character(groups),cex.names=cex.names);x2
      x4<-right_margin(x=groups);x4
      ...
      if(save==TRUE){ png(filename=file.path(dir,"BoxPlot.png"), units="px", 
                          width=width, height=ifelse(width<=800,width,800), pointsize=15)}
      # par(op)
      # op <- par(no.readonly = TRUE)
      par(mar = c(x1,6,3, x2), xaxs="i")#par(oma=c(0.5,0,0.5,x4/2+3)))
      box<-graphics::boxplot(data, col=colorGroup2(groups)[groups], names=sampleLabels, 
                             notch=FALSE, horizontal=FALSE, outline=FALSE,
                             main="", ylab="", xlab="",xlim=c(0,ncol(data)+1),
                             las=2, font=1, cex.axis=1.2, cex.names=cex.names,cex=1)
      mtext(side=2, text="Intensity", font=1, line=3, cex=1.2)
      title(main=ifelse(is.null(title), "", title), font.main=1, cex.main=1.3, line=1.1)
      if(legend==FALSE){ x4<-1};x4
      if(legend==TRUE){
         legend(par("usr")[2],par("usr")[4], bty="n",xpd=NA, legend=levels(groups), pch = 22, cex=cex.names,
   ...
   ##  [20] VIOLIN PLOT 
   ##------------------------------
   plotViolin <- function(data, groups=NULL, sampleLabels=NULL, cex.names=1, title=NULL, legend=TRUE, dir=".", save=FALSE){
      ...
      par(mar = c(x1,6,3, x2), xaxs="i")#, yaxs="i")#, oma=c(1,1,1,1))
      vio<-vioplot::vioplot(plotData, col=colorGroup2(groups)[groups], names=sampleLabels,
                            main="", ylab="", xlab="",xlim=c(0,length(plotData)+1),
                            las=2, font=1, cex.axis=1.2, cex.names=cex.names,cex=1)
   ...
   plotPCA <- function(data, groups=NULL, sampleLabels=NULL, title=NULL, top=500, stdize=TRUE, cex.names=1,
      ...
      ## plot margins
      x1<-left_margin(x=sampleLabels);x1
      x4<-right_margin(x=groups);x4
      
      
      if(save==TRUE){ if(!dir.exists(dir)){dir.create(dir,recursive=TRUE)} }
      if(save==TRUE){png(filename=file.path(dir,"PCAplot.png"), units="px", 
                         width=750, height=600, pointsize=15)}
      # par(op)
      # op <- par(no.readonly = TRUE)
      par(mar = c(6,6,3, x4), par(oma=c(0.5,0,0.5,x4/2+3)))   
      ...
      text(labels=sampleLabels, x=pca$x[,dims[1]], y=pca$x[,dims[2]],
           cex=cex.char, adj=ifelse(max.char < 3,-0.6, ifelse(max.char < 5, -0.4,-0.15)), 
   ...
   plotDendrogram <- function(data, groups=NULL, sampleLabels=NULL, top=500, stdize=TRUE, 
      ...
      if(save==TRUE){png(filename=file.path(dir,"Dendrogram.png"), units="px", 
                         width=width, height=500, pointsize=15)}
      
      # op <- par(no.readonly = TRUE)
      # par(xpd = T, mar = par()$mar + c(3,0,2,5))
      par(xpd = T, mar=c(x1,3,3,x2),oma=c(0.5,0.5,0.5,0.8), yaxt="n",xaxs="i")
      ...
      ClassDiscovery::plotColoredClusters(hc, labs=sampleLabels, cols=colorGroup2(groups)[groups],
                                          lwd=1.5, las=2, cex.axis=1, xlab="",ylab="", font=1, cex=cex.names, 
                                          line=-0.5, xlim=c(0,length(data)+2))
      title(main=ifelse(is.null(title),"",title), font.main=1, line=2,cex=1.5)
   ...
   make_qc_report <- function(normList, norm.meth="cycloess", groups=NULL, batch=NULL, sampleLabels=NULL, 
                              stdize=TRUE, top=500, dims=c(1,2), cex.dot=2, cex.names=1,clust.metric="euclidean",
                              clust.meth="complete",xlim=NULL, ylim=NULL,
      ...
      if(!is.null(batch)){
         ...
         if(nchar(max(as.character(batch))) < nchar(max(as.character(groups)))){
            dif <- (max(nchar(as.character(groups)))-max(nchar(as.character(batch))))*3;dif
            batch2<-paste0(batch,paste0(rep(" ",dif),collapse=""));batch2
            batch2<-make_factor(x=batch2);batch2
         } else {batch2 <- batch }
         
         if(save==TRUE){ 
            width=round(0.0191*length(groups)^2 + 12.082*length(groups) + 671.75,0);print(width)
            png(filename=file.path(dir,"BoxPlot2.png"), units="px", width=width, height=750, pointsize=15) }
            o <- order(batch2);batch2[o]
            box2 <- plotBoxplot(data=data[,o],groups=batch2[o],sampleLabels=sampleLabels[o],title="Box Plot (batch)",legend=legend,dir=dir,save=FALSE)
         if(save==TRUE){ dev.off() }
      }
      
      ## VIOLIN PLOTS
      vio <- plotViolin(data=data, groups=groups, sampleLabels=sampleLabels, cex.names=cex.names, title="Violin Plot", 
                        legend=legend, dir=dir, save=save)
      if(!is.null(batch)){
         width=round(0.0191*length(groups)^2 + 12.082*length(groups) + 671.75,0);print(width)
         if(save==TRUE){ png(filename=file.path(dir,"ViolinPlot2.png"), units="px", width=width, height=750, pointsize=15) }
            o <- order(batch2);o ## order by batch 
            vio2 <- plotViolin(data=data[,o],groups=batch2[o],sampleLabels=sampleLabels[o], cex.names=cex.names, title="Violin Plot (batch)",
                               legend=legend, dir=dir, save=FALSE)
         if(save==TRUE){ dev.off() }
      }
      ...
      if(!is.null(batch)){
         width=round(0.0191*length(groups)^2 + 12.082*length(groups) + 671.75,0);print(width)
         if(save==TRUE){ png(filename=file.path(dir,"PCAplot2.png"), units="px",width=width, height=650, pointsize=15) }
         o<-order(batch);batch[o]
         pca2 <- plotPCA(data=data[,o],groups=batch[o],sampleLabels=sampleLabels[o],title="PCA Plot (batch)", top=top,stdize=stdize, dims=dims,
      ...
      dendro <- plotDendrogram(data=data,groups=groups,sampleLabels=sampleLabels,top=top,stdize=stdize,
                               ...
                               cex.names=cex.names,xlim=NULL,title="Sample Dendrogram",legend=legend,dir=dir,save=save)
      if(!is.null(batch)){
         if(save==TRUE){ 
            width=round(0.0191*length(groups)^2 + 12.082*length(groups) + 671.75,0);print(width)
            png(filename=file.path(dir,"Dendrogram2.png"), units="px", width=width, height=500, pointsize=15) }
               o<-order(batch2);batch2[o]
               dendro2 <- plotDendrogram(data=data[,o],groups=batch2[o],sampleLabels=sampleLabels[o],top=top,stdize=stdize,
                                         clust.metric=clust.metric,clust.meth=clust.meth,
                                         cex.names=cex.names,xlim=NULL,title="Sample Dendrogram (batch)",legend=legend,dir=dir,save=FALSE)
         if(save==TRUE){ dev.off() }
      }
...
{ ## LIMMA DE FUNCTIONS
   ...
   extract_limma_results <- function(efit, data, annot, min.pval=0.055, min.lfc=1, adj.method="BH",
      ...
      if(save==TRUE){ 
         ...
         utils::write.csv(comboStats2,file=file.path("logs",filename),row.names=FALSE)
...
{ ## DE PLOT FUNCTIONS
   ...
   glimmaVolcanoPlot <- function(stats, comparison, data, annot, groups, min.pval=0.055, min.lfc=1, 
      ...
      suppressWarnings(writeLines(as.character(tmptxt), file(fixfile), sep="\n"))
   } ## GLIMMA VOLCANO PLOT
   ...
   pvalueHistogram <- function(stats, comparison=NULL){
      ...
      par(mar=c(6,7,5,5))
      brk <- seq(from=0, to=1, by=0.05)
      graphics::hist(stats$P.Value, col="dodgerblue", breaks=brk, 
                     main="", xlab="", ylab="", freq=TRUE, 
      ...
      par(mar=c(6,7,5,5))
      brk <- seq(from=0, to=1, by=0.05)
      graphics::hist(stats$adj.P.Val, col="dodgerblue", breaks=brk, 
                     main="", xlab="", ylab="", freq=TRUE,
                     xlim=c(0,1), las=1,cex.axis=1.2)
      title(line=1, main=comparison, cex.main=1.3)
